...
ko.components.register('cellMainCategory', {
 ...
 ko.components.register('cellMainFilter', {
    ...
    <!-- ko text:title --> <!-- /ko --> <span class="caret"></span>
					...
					<th></th>
					<th></th>
					<th><button type="button" class="btn btn-default btn-block" data-bind="click:$root.goToAddApplying">新增</button></th>
				</tr>
			</thead>`
});
 ...
 ko.components.register('cellMainSubmitBtn', {
    ...
    template:"<td>\
						<button class='btn btn-primary btn-block' data-bind='visible:((!(item.C3_541449606438==\"Y\")&&item.C3_541449538456==\"Y\") && item.C3_545927244809 == \"Y\" ),click:$root.newp'>附件</button>\
                            <button class='btn btn-primary btn-block' data-bind='visible:(item.C3_541449606438==\"Y\"||(item.C3_541449538456==null||item.C3_541449538456==\"N\")),click:$root.submite'>提交</button>\
						</td>\
					<td>\
...
// 审批中搜索栏
 ko.components.register('cellMainFilterSearch', {
    ...
    <!-- ko text:title --> <!-- /ko --> <span class="caret"></span>
					...
					<th><input type="checkbox" data-bind="checked:allSelected,event:{change:$root.selectAllChange}" /></th>
					<th></th>
					<th></th>
					<th></th>
					<th><button class='btn btn-primary btn-block' data-bind='click:function(){$root.goToApplyDetailPage($index)}'>审批</button></th>
				</tr>
			</thead>`
});
...
// 审判中退回按钮组
 ko.components.register('cellMainRefuseBtn', {
    ...
    template:`<td>\
            ...
            <td>\
                <input type="checkbox" data-bind="checked:item.selected,event:{change:$root.checkboxChange($index)}" />
            </td>`
});
...
//分页
 ko.components.register('pageSelectCpt', {
  ...
  <ul class="pagination">
    <li>
      <a href="" data-bind="click:$root.pageUp" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </a>
    </li>
    <li>
      <a href="" data-bind="click:$root.pageDown" aria-label="Next">
